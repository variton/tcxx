...
public:
  ...
  tl::expected<void, ErrTypeInfo> compare_id(int jobs) const noexcept {
    if (jobs == workers_) {
...
public:
  ...
  tl::expected<void, ServerErrorInfo> compare_id(int val) const noexcept {
    Server server{2};
...
public:
  ...
  tl::expected<void, typename T::ErrTypeInfo>
